...
impl IoTRegistry {
    ...
    ) -> Result<DataContribution, String> {
        let device = self
            .devices
            .get_mut(&contribution.device_id)
    ...
    pub fn get_stats(&self) -> IoTRegistryStats {
        ...
        let type_distribution: HashMap<String, u64> = self
            .device_by_type
            .iter()
...
/// Get device by ID
pub async fn get_iot_device(data: web::Data<IoTState>, path: web::Path<String>) -> impl Responder {
...
pub async fn submit_data(
    ...
    let registry = data.registry.read().await;
    let device_multiplier = registry
    ...
    match registry.record_contribution(contribution) {
        Ok(contribution) => {
            info!(
                "Data contribution recorded: {} from {} ({} bytes, {:.4} EDGE)",
...
pub async fn verify_device(data: web::Data<IoTState>, path: web::Path<String>) -> impl Responder {
    let device_id = path.into_inner();
    let mut registry = data.registry.write().await;
...
pub async fn get_iot_stats(data: web::Data<IoTState>) -> impl Responder {
    let registry = data.registry.read().await;
...
pub async fn get_recent_contributions(
    ...
    let contributions: Vec<ContributionResponse> = registry
        .contributions
        .iter()
        .rev()
        .take(limit)
...
pub async fn get_device_leaderboard(
    ...
    match sort_by {
        "contributions" => {
            devices.sort_by(|a, b| b.total_contributions.cmp(&a.total_contributions))
        }
        "reputation" => {
            devices.sort_by(|a, b| b.reputation_score.partial_cmp(&a.reputation_score).unwrap())
        }
        "data" => devices.sort_by(|a, b| b.total_data_bytes.cmp(&a.total_data_bytes)),
        _ => devices.sort_by(|a, b| {
            b.total_rewards_earned
                .partial_cmp(&a.total_rewards_earned)
                .unwrap()
        }),
    }
...
pub fn configure_iot_routes(cfg: &mut web::ServiceConfig) {
    cfg
        ...
        .route(
            ...
            web::post().to(register_iot_device),
        )
        .route(
            "/api/iot/devices/{device_id}",
            web::get().to(get_iot_device),
        )
        .route(
            "/api/iot/devices/{device_id}/verify",
            web::post().to(verify_device),
        )
        .route(
            "/api/iot/devices/owner/{owner}",
            web::get().to(get_devices_by_owner),
        )
        .route(
            "/api/iot/devices/leaderboard",
            web::get().to(get_device_leaderboard),
        )
        // Data contributions
        .route("/api/iot/data/submit", web::post().to(submit_data))
        .route(
            "/api/iot/data/recent",
            web::get().to(get_recent_contributions),
        )
        // Device operations
