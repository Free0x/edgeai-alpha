...
impl Storage {
    ...
    pub fn put_block(&self, block: &Block) -> Result<(), String> {
        let cf_blocks = self.db.cf_handle(CF_BLOCKS).ok_or("CF_BLOCKS not found")?;
        let cf_hashes = self
            .db
            .cf_handle(CF_BLOCK_HASHES)
    ...
    pub fn put_account(&self, account: &Account) -> Result<(), String> {
        ...
        self.db
            .put_cf(&cf_accounts, account.address.as_bytes(), &data)
    ...
    pub fn put_data_entry(&self, entry: &DataEntry) -> Result<(), String> {
        ...
        self.db
            .put_cf(&cf_data, entry.hash.as_bytes(), &data)
    ...
    pub fn put_metadata(&self, metadata: &ChainMetadata) -> Result<(), String> {
        ...
        batch.put_cf(
            &cf_meta,
            META_TOTAL_BLOCKS,
            &metadata.total_blocks.to_be_bytes(),
        );
        batch.put_cf(
            &cf_meta,
            META_DIFFICULTY,
            &metadata.difficulty.to_be_bytes(),
        );
        batch.put_cf(
            &cf_meta,
            META_BLOCK_REWARD,
            &metadata.block_reward.to_be_bytes(),
        );
        batch.put_cf(
            &cf_meta,
            META_DATA_REWARD_BASE,
            &metadata.data_reward_base.to_be_bytes(),
        );
        batch.put_cf(
            &cf_meta,
            META_LAST_BLOCK_TIME,
            &metadata.last_block_time.to_be_bytes(),
        );
    ...
    pub fn put_data_entries_batch(
        &self,
        entries: &std::collections::HashMap<String, DataEntry>,
    ) -> Result<(), String> {
        let cf_data = self
            .db
            .cf_handle(CF_DATA_REGISTRY)
    ...
    fn get_u64(&self, cf: &rocksdb::ColumnFamily, key: &[u8]) -> Option<u64> {
        match self.db.get_cf(cf, key) {
            Ok(Some(bytes)) if bytes.len() == 8 => Some(u64::from_be_bytes(bytes.try_into().ok()?)),
            _ => None,
        }
    }
    ...
    fn get_i64(&self, cf: &rocksdb::ColumnFamily, key: &[u8]) -> Option<i64> {
        match self.db.get_cf(cf, key) {
            Ok(Some(bytes)) if bytes.len() == 8 => Some(i64::from_be_bytes(bytes.try_into().ok()?)),
            _ => None,
        }
    }
